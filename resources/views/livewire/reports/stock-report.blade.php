<div>
    ...
    <div wire:loading.remove.delay id="stockReportContent">
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                            ...
                            <div class="flex-grow-1 ms-3">
                                ...
                                <h4 class="mb-0">{{ number_format($summary['total_products']) }}</h4>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                            ...
                            <div class="flex-grow-1 ms-3">
                                ...
                                <h4 class="mb-0">{{ number_format($summary['total_stock']) }}</h4>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                            ...
                            <div class="flex-grow-1 ms-3">
                                ...
                                <h4 class="mb-0">{{ number_format($summary['low_stock']) }}</h4>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                            ...
                            <div class="flex-grow-1 ms-3">
                                ...
                                <h4 class="mb-0">{{ number_format($summary['out_of_stock']) }}</h4>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <!-- Stock Alerts -->
        @if(count($alerts['danger']) > 0 || count($alerts['warning']) > 0)
                ...
                <div class="card-header bg-gradient-danger text-white border-0">
                    <h5 class="mb-0 fw-bold">
                        ...
                        <span class="badge bg-white text-danger ms-2">
                            {{ count($alerts['danger']) + count($alerts['warning']) }}
                        </span>
                    </h5>
                </div>
                <div class="card-body p-0">
                    @if(count($alerts['danger']) > 0)
                            ...
                            <div class="d-flex align-items-start p-3 bg-danger bg-opacity-10">
                                ...
                                <div class="flex-grow-1 ms-3">
                                    <h6 class="fw-bold text-danger mb-2">
                                        Critical: {{ count($alerts['danger']) }} Product(s) Out of Stock
                                    </h6>
                                    <div class="d-flex flex-wrap gap-2">
                                        @foreach($alerts['danger'] as $product)
                                            <span class="badge bg-danger">{{ $product['name'] }}</span>
                                        @endforeach
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="d-flex align-items-start p-3 bg-warning bg-opacity-10">
                                ...
                                <div class="flex-grow-1 ms-3">
                                    <h6 class="fw-bold text-warning mb-2">
                                        Warning: {{ count($alerts['warning']) }} Product(s) Low on Stock
                                    </h6>
                                    <div class="d-flex flex-wrap gap-2">
                                        @foreach($alerts['warning'] as $product)
                                            <span class="badge bg-warning">
                                                {{ $product['name'] }} 
                                                <span class="badge bg-white text-warning ms-1">{{ $product['stock'] }}</span>
                                            </span>
                                        @endforeach
                                    </div>
                                </div>
                            ...
                            @forelse($products as $product)
                                <tr>
                                    ...
                                    <td data-label="Category">
                                        <span class="badge bg-label-info">{{ $product->category?->name ?? 'Uncategorized' }}</span>
                                    </td>
                                    <td class="text-end" data-label="Stock">
                                        <span class="fw-semibold">{{ number_format($product->stock) }}</span>
                                    </td>
                                    ...
                                    <td data-label="Last Updated">{{ $product->updated_at->diffForHumans() }}</td>
                                </tr>
                            @empty
                                <tr>
    ...
    <script>
        ...
        function initStockCharts() {
            ...
            const categoryStock = @json($categories);
